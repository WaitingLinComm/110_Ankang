...
void coarseTO_estimator_v3(vector<double> &r_I,  vector<double> &r_Q, int L_r, vector<double> &q_I, vector<double> &q_Q, int L_q, int sps, vector<double> &y1_I, vector<double> &y1_Q,  vector<int> &start_burst_position_index, vector<int> &end_burst_position_index)
{  
    clock_t t1, t2;
    // realize : q(end : -1 : 1)
    ...
    complex_convolution_with_norm(y1_norm2, L_r ,q_flip_I, conj_q_flip_Q, L_q, r_I, r_Q, L_r);
    // t2 = clock();
    ...
    // sol2:
    find_burst_interval(start_burst, end_burst, y1_norm2, sps);
    //t2 = clock();
