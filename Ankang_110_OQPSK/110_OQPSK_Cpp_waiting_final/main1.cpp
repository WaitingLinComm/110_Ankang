...
// Execute instruction: g++ -o d main1.cpp -lfftw3 -lm
...
#include "complex_convolution.h" // used in main and "OQPSK_frontendTx.h"
...
{ 
    ...
    // int filelength = 6.25 * pow(10, 6); // Number of used samples 
    ...
    double coarse_pilot_position_burst1;  
    coarse_pilot_position_burst1 = coarse_pilot_position[1-1]; // First test with burst 9: The position of highest peak
    ...
    //cout << "time_recording_buffer: " << time_recording_buffer[0][0] << time_recording_buffer[0][1] << "\n";  
/*
    //cout << "size of b_i_hat_p_buffer: " << b_i_hat_p_buffer.size() << "\n";  
    // for(int j = 0; j < b_i_hat_p_buffer.size(); j++){
    //     printf("[%d]: %f\n", j, b_i_hat_p_buffer[j]);
    // }  
*/       
#endif 
...
//////////////  DecisionProcess  ////////////////
    ...
    double SNR_dB;
    SNR_dB = SNRestimation(DataType, b_i_hat_p_buffer, b_q_hat_p_buffer);
    ...
    //void ErrorRateCalculation_v2(vector<double> &SER_BER_errorpilot, string &mode_type, vector<double> &pilotSym, vector<double> &ReceivedPilotSym)
    ErrorRateCalculation(SER_BER_errorpilot, DataType, pilotPattern_vec, ReceivedSym_d);
