...
{ 
    ...
    // fc =  -1010131, -679997, -338745, 0, 341796, 677490, 1019287, 1361083      
    // Tx info.
    ...
    //int filelength = pow(10, 8);         // Number of used samples 
...
cout<<"all file is read"<<"\n";

int total_samples = filelength;	
int samples_per_slice = 1e8;    
int number_of_slice = ceil((double)total_samples/(double)samples_per_slice);

// cout << total_samples <<"\n";
// cout << ceil(total_samples/samples_per_slice) <<"\n";
// cout << (total_samples/samples_per_slice) <<"\n";
cout << "number_of_slice: " << number_of_slice <<"\n"; // number_of_slice = 4
///////////// Divide data into different slices and do process ////////////////////////
int j = 0;
for (int j = 0; j < number_of_slice; j++){

    if(j == number_of_slice - 1){
        for (int i = 0 + j * samples_per_slice/2; i < filelength/2; i++){
        ...
        for (int i = 0 + j * samples_per_slice/2; i < (j+1) * samples_per_slice/2; i++){
            rawDataI.push_back(Data_from_file[2*i]); 
            rawDataQ.push_back(Data_from_file[2*i + 1]);
        }
    }
    ...
    // test
    //int samples_per_slice = 0.5*1e6;
    // int number_of_slice = 2;
    // ofstream OQPSK_demod_result_1;
    // ofstream OQPSK_demod_result_2;
    ...
    }//end for (int j = 0; j < number_of_slice; j++){
}

    // for(int j = 0; j < ReceivedSym_d.size(); j++){    
    //     printf("%f", ReceivedSym_d[j]);
    // }