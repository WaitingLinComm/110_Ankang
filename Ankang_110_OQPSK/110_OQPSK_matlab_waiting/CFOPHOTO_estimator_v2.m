function [cfo_hat, pho_hat, d, y2_norm2_buffer, vCFO_buffer] = CFOPHOTO_estimator_v2(v, q, K, Ts)
% CFOPHOTO_estimator_v2.m
% CFOPHOTO_estimator_v2.m is the speed up version of CFOPHOTO_estimator.m
% :reduce the needed while loop and for loop. Discriminant is also added to break the loop earlier.
% 
...
% while(cfo_step > 1e-7)
    ...
    for i =10 : length(vCFO)-11 % or equal to using i = 10 % speed up
    %for i =1 : length(vCFO) % original
    ...
    % check: if the consecutive update of cfo, pho is not large => break the loop % speed up
    if iterations == 1
    ...
    if(cfo_differ <= 0.1 || pho_differ <= 0.1)
        break;
    end   
end
