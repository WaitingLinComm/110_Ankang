...
void OQPSK_frontendTx(vector<double> &OQPSKwaveform_I, vector<double> &OQPSKwaveform_Q, vector<double> &m, vector<double> &Pulseshaping, int FilterLength, int Pattern_Length, int sps, int bitorder, int bitmap, int IQinterlace)
{
    ...
    OQPSK_bitReorganization(bitorder, m, b, Pattern_Length);

    //////////////////////       Bit Mapping       /////////////////////
    //// Change bit value (1 / 0) to voltage (-1 / 1) or bit value (1 /  0) to voltage (1 / -1) ////
    vector<double> psi;
    psi.reserve(Pattern_Length);
    OQPSK_bitMapping(bitmap, b, psi, Pattern_Length); 
    ...
    OQPSK_I_Q_interlace(IQinterlace, psi, l_I, l_Q, Pattern_Length);
    ...
    OQPSK_Upsample(l_I, u_I, N, L_u);
    OQPSK_Upsample(l_Q, u_Q, N, L_u);
    ...
    OQPSK_complex_convolution(OQPSKwaveform_I, OQPSKwaveform_Q, L_OQPSKwaveform ,Pulseshaping, Pulseshaping_Q, FilterLength, u_I, u_Q, L_u);
}