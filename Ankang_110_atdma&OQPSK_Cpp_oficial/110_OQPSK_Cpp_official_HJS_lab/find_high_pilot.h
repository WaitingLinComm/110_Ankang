...
void find_high_pilot(vector<int> &pilot_position , vector<double> &MF, int sps)
{
    //use the mean of following 50 symbol as thresold to detect the pilot head
    int S = 100; 
    double runnig_sum_temp = sum(MF, 0 ,S * sps);
    ...
    double scalar = 5;// MF value should be higher than scalar*running_mean to be candidate for pilot head
    int count_pilot = 0;
    for (int testing_index = 0 ; testing_index < L_MF - 2000 * sps; testing_index++) //test all MF point
    {
        if( ((S * sps * MF[testing_index]) > (scalar * runnig_sum_temp)) && switch_for_detect_head == 1)
        {
            ...
            {
                index_for_detect_end = testing_index + 30* 16  * sps  ; 
                switch_for_detect_end = 0 ; 
            }
            else if (40 * sps * MF[testing_index] > scalar * sum_detect_2)
            {
                index_for_detect_head = testing_index + 30* 16 * sps;
                double max_value;
                int max_index;
                MAX(detect_temp, max_value, max_index);
                pilot_position.push_back( max_index + testing_index );
