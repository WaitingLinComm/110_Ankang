...
void OQPSK_CFOPHOTO_estimator_v2(vector<double> &CFOhat_PHOhat_d, vector<double> &v_I, vector<double> &v_Q, int Lv, vector<double> &q_I, vector<double> &q_Q, int Lq, int K, double Ts)
{   
    ...
    //while(cfo_step > 1e-7){    
        ...
        for (int i = 1; i <= L_vCFO - 11; i++) // or i == 10
        {
            ...
            OQPSK_Flip_vec(p_flip_I, p_I, Lp);
            OQPSK_Flip_vec(p_flip_Q, p_Q, Lp);
    ...
    */    
        ...
        */
        ...
        for(int i = 9; i < L_vCFO-11; i++) // for(int i = 9; i < L_vCFO-11; i++)
        {
            for(int j = 0; j < Lv; j++){ // for(int j = 2000; j < Lv-1000; j++)
                if(y2_norm2[i][j] > max){
                    max = y2_norm2[i][j];
                    cfo_estimate_id = i;
                    to_estimate_id = j;
