...
vector<double> OQPSK_main_5(int pilotType, vector<int> &pilot_manual, double fc, double fsy, double fs, const char *filename, int filelength, vector<int> &demd_symbol_per_burst, vector<int> &coarse_pilot_position, vector<int> &end_burst_position)
{ 
    ...
    struct  timeval t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, t18, t19, t20;
    struct  timeval tIteration_start, tIteration_end;
    struct  timeval total_start, total_end;
    
    unsigned long diff_t1, diff_t3 ,diff_t5, diff_t7, diff_t9, diff_t11, diff_t13, diff_t15, diff_t17, diff_t19;
    unsigned long diff_tIteration;
    unsigned long diff_total;

    double diff_t1_result, diff_t3_result, diff_t5_result, diff_t7_result, diff_t9_result, diff_t11_result, diff_t13_result, diff_t15_result, diff_t17_result, diff_t19_result;
    double diff_tIteration_result;
    double diff_total_result;

    // gettimeofday(&total_end,NULL);
    // diff_total =  1000000 * (total_end.tv_sec-total_start.tv_sec)+total_end.tv_usec-total_start.tv_usec;//     
    // double diff_total_result = diff_total/1000000.0 ;
    ...
    gettimeofday(&t1,NULL); //t1 = clock();
    OQPSK_DataConversion(filename, filelength, rawDataI, rawDataQ);
    gettimeofday(&t2,NULL); //t2 = clock();
    diff_t1 =  1000000 * (t2.tv_sec - t1.tv_sec) + t2.tv_usec - t1.tv_usec;//     
    diff_t1_result = diff_t1/1000000.0 ;

    //printf("Data conversion is done in %1f(sec).\n", (t2-t1)/(double)(CLOCKS_PER_SEC));
    ////////////////////////// DC Blocker (Remove DC offset)  /////////////////////// 
    gettimeofday(&total_start,NULL); //total_start = clock();  // Start time calculation from here.(Time of DataConversion don't need to count in)
    vector<double> Data_withoutDC_I, Data_withoutDC_Q;
    Data_withoutDC_I.reserve(SigLength); Data_withoutDC_Q.reserve(SigLength); 

    gettimeofday(&t3, NULL); // t3 = clock();
    OQPSK_DC_Blocker(Data_withoutDC_I, Data_withoutDC_Q, SigLength, rawDataI, rawDataQ);
    rawDataI.clear(); rawDataQ.clear(); 
    gettimeofday(&t4, NULL); // t4 = clock();
    diff_t3 =  1000000 * (t4.tv_sec - t3.tv_sec) + t4.tv_usec - t3.tv_usec;//     
    diff_t3_result = diff_t3/1000000.0;
    ...
    gettimeofday(&t6, NULL); // t6 = clock();
    diff_t5 =  1000000 * (t6.tv_sec - t5.tv_sec) + t6.tv_usec - t5.tv_usec;//     
    diff_t5_result = diff_t5/1000000.0 ;
    ...
    gettimeofday(&t8, NULL); // t8 = clock();
    diff_t7 =  1000000 * (t8.tv_sec - t7.tv_sec) + t8.tv_usec - t7.tv_usec;//     
    diff_t7_result = diff_t7/1000000.0;
    ...
    gettimeofday(&t10, NULL);  // t10 = clock();
    diff_t9 =  1000000 * (t10.tv_sec - t9.tv_sec) + t10.tv_usec - t9.tv_usec;//     
    diff_t9_result = diff_t9/1000000.0;
    ...
    gettimeofday(&t11, NULL); // t11 = clock(); 
    int down_sample = 4;
    ...
    gettimeofday(&t12, NULL);  // t12 = clock(); 
    diff_t11 =  1000000 * (t12.tv_sec - t11.tv_sec) + t12.tv_usec - t11.tv_usec;//     
    diff_t11_result = diff_t11/1000000.0;    
    ...
    gettimeofday(&tIteration_start, NULL);     //tIteration_start = clock(); 
    int L_b_hat_d_burst; 
    for (int i = 0; i < number_of_bursts; i++){
        //////////////////////   Get processed data   ////////////////////// 
        // int K = 2000;
    ...
    gettimeofday(&tIteration_end, NULL);  //tIteration_end = clock(); 
    diff_tIteration =  1000000 * (tIteration_end.tv_sec - tIteration_start.tv_sec) + tIteration_end.tv_usec - tIteration_start.tv_usec;//     
    diff_tIteration_result = diff_tIteration/1000000.0; 
...
//////////////  DecisionProcess  ////////////////
    ...
    // for pilot + data
    gettimeofday(&t13, NULL);  //t13 = clock();
    ...
    gettimeofday(&t14, NULL);  // t14 = clock();
    diff_t13 =  1000000 * (t14.tv_sec - t13.tv_sec) + t14.tv_usec - t13.tv_usec;//     
    diff_t13_result = diff_t13/1000000.0;  
    ...
    // b_hat_p.clear(); 
    // for pilot + data
    gettimeofday(&t15, NULL); // t15 = clock();
    vector<double> m_hat_d;
    int L_m_hat_d = L_b_hat_d;
    m_hat_d.reserve(L_m_hat_d);
    OQPSK_bitReorganizationRx(m_hat_d, b_hat_d, L_m_hat_d);
    gettimeofday(&t16, NULL);  // t16 = clock();
    diff_t15 =  1000000 * (t16.tv_sec - t15.tv_sec) + t16.tv_usec - t15.tv_usec;//     
    diff_t15_result = diff_t15/1000000.0; 
    ...
    // m_hat_p.clear();
    // for pilot + data
    gettimeofday(&t17, NULL); // t17 = clock();
    ...
    m_hat_d.clear();
    gettimeofday(&t18, NULL);  // t18 = clock();
    diff_t17 =  1000000 * (t18.tv_sec - t17.tv_sec) + t18.tv_usec - t17.tv_usec;//     
    diff_t17_result = diff_t17/1000000.0; 
    
    gettimeofday(&total_end,NULL); //total_end = clock();
    diff_total =  1000000 * (total_end.tv_sec-total_start.tv_sec)+total_end.tv_usec-total_start.tv_usec;//     
    diff_total_result = diff_total/1000000.0 ;
