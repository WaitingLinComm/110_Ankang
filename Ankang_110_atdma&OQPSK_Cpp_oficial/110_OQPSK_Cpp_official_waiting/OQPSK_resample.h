 ...
 * 
 */
void OQPSK_resample(vector<double> &ResampledSig_I, vector<double> &ResampledSig_Q, int SigLength_afterResample, vector<double> &InSig_I, vector<double> &InSig_Q, int SigLength_beforeResample, double N, double window)
{   
    ...
    for(int n2 = 1; n2 <= SigLength_afterResample; n2++){              
        ...
        if(n1_match > window & n1_match < SigLength_beforeResample - window){
            for(int n1 = n1_match - window; n1 <= n1_match + window; n1++){
                inner_sum_I += InSig_I[n1 - 1] * OQPSK_sinc(n2/N - n1); // #include "sinc.h"
                inner_sum_Q += InSig_Q[n1 - 1] * OQPSK_sinc(n2/N - n1);   
            }   
